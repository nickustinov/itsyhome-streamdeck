...
import { LightAction } from "../../src/actions/light";
...
describe("LightAction", () => {
  ...
  describe("onDidReceiveSettings", () => {
    ...
    it("uses custom port", async () => {
      ...
      const ev = {
        action: createMockAction(),
        payload: { settings: { target: "Lamp", port: 1234 } },
      };
  ...
  describe("onKeyDown", () => {
    ...
    it("toggles light and updates cache optimistically", async () => {
      mockClient.getDeviceInfo.mockResolvedValue({
        name: "Lamp", type: "light", reachable: true, state: { on: true },
      });
  ...
  describe("state handling", () => {
    it("defaults isOn to false when state.on is undefined and no brightness", async () => {
      mockClient.getDeviceInfo.mockResolvedValue({
